...
void ordenarPantallaPrecio(Pantalla *pPantalla,int limite);
    ...
    while(opcion!=6)
    {
        ...
        switch(opcion)
        {
            ...
            case 7: //informes
            {
                ...
                ordenarPantallaPrecio(visores,1);


                //No copie el while bien del pibe que lo hizo
...
{
    flag=0;
    for(j=0;j<limite-1;j++) //no comparar elementos que estan vacios y forzar la proxima iteracion
    {
        if(pPantalla[j].precio<pPantalla[j+1].precio)
        {
            buffer=pPantalla[j];
            pPantalla[j]=pPantalla[j+1];
            pPantalla[j+1]=buffer;
            flag=1;
        }
        else if(pPantalla[j].precio==pPantalla[j+1].precio)//si los precios son iguales ordeno por nombrea abajo
        {
            if(strncmp(pPantalla[j].nombre,pPantalla[j+1].nombre,50)>0) //ordeno por nombre si los precios son iguales
            {
                buffer=pPantalla[j];
                pPantalla[j]=pPantalla[j+1];
                pPantalla[j+1]=buffer;
